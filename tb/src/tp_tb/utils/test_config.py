...
def input_args_from_config(config) :
    with open(config, "r") as infile :
        ...
        if "testbench_config" not in data :
            raise Exception(f"ERROR Top-level node \"testbench_config\" not found in provided test config file (={config}).")
        test_config = data["testbench_config"]
        if "input_args" not in test_config :
            raise Exception(f"ERROR \"input_args\" not found in provided test config file (={config}).")
...
def check_config_file(config_file) :
    ...
    if not file_ok :
        return False, f"ERROR Test config file (={config_file}) could not be found or opened"
    ...
    if not file_ok :
        return False, f"ERROR Expected test module (={str(expected_test_module)}) not found"
    file_ok = expected_top_level.exists() and expected_top_level.is_file()
    if not file_ok :
        return False, f"ERROR Expected top level verilog (={str(expected_top_level)}) not found"
    file_ok = expected_makefile.exists() and expected_makefile.is_file()
    if not file_ok :
        return False, f"ERROR Expected test Makefile (={str(expected_makefile)}) not found"
