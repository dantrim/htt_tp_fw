...
def dump_evt_file(
    ...
    with open(input_filename, "rb") as input_file:
        for _ in range(0, filesize, N_BYTES_PER_WORD):
            ...
            ##
            if (
                data_word.is_metadata
                and (not data_word.is_start_of_event())
            ...
            ##
            if data_word.is_start_of_event():
                header = DataFormat.BitFieldWordValue(
                    DataFormat.EVT_HDR1, data_word.contents
                )
                ...
                if do_parse:
                    ...
                    if module_word_start_idx >= 0:
                        ...
                        if module_idx < N_WORDS_FOR_MODULE_HEADER:
                            module_field_desc = [DataFormat.M_HDR, DataFormat.M_HDR2][
                                module_idx
                            ]  # DataFormat breaks its own naming conventions
                    ...
                    if s_in_module and (module_word_start_idx < 0):
                        ...
                        cluster_data_str = str(data_word).replace("0x", "")[
                            1:
                        ]  # remove '0x' string and skip metadata flag
                        cluster_words = []
                        if module_type == "pix":
