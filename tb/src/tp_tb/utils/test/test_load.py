...
def main() :
    ...
    parser.add_argument("-f", "--filename", default = "/home/dantrim/work/tdaq-htt-firmware/testvecs/20200124/BoardToBoardInput_AMTP0_Pixel0.evt")
    parser.add_argument("--ids", action = "store_true", default = False,
        help = "Print L0IDs of loaded events"
    )
    parser.add_argument("-n", "--n-events", default = -1,
        help = "Number of events to load [default:all]"
    )
    parser.add_argument("--l0id", default = "",
        help = "Load data for specific l0id (hex string)"
    )
    parser.add_argument("--dump", action = "store_true", default = False)
    args = parser.parse_args()

    loaded_events = events.load_events_from_file(filename = args.filename, n_to_load = int(args.n_events))
