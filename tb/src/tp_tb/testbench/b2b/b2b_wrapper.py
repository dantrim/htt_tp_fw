...
class B2BWrapper(Wrapper):
    ...
    def compare_port_with_expected(self, port_num, expected_events):
        ...
        if not test_l0ids:
            ...
            n_common, n_exp_not_obs, n_obs_not_exp = (
                len(common_l0ids),
                len(in_exp_not_obs),
                len(in_obs_not_exp),
            )
            n_rows = max([n_common, n_exp_not_obs, n_obs_not_exp])
            for l0id_check in [common_l0ids, in_exp_not_obs, in_obs_not_exp]:
        ...
        for ievent in range(len(expected_events_sorted)):
            ...
            headers_equal = (
                event_header_words_expected == event_header_words_observed
            )  # this does word-by-word comparison
            ...
            if not headers_equal:
                ...
                for header_row in header_field_names:
                    for field in header_row:
                        ...
                        data.append(
                            ["", field, hex(obs), hex(exp), err_str[obs == exp]]
                        )
                        if not obs == exp:
            ...
            footers_equal = (
                event_footer_words_expected == event_footer_words_observed
            )  # this does word-by-word comparison
            ...
            if n_modules_equal:
                ...
                for imod in range(n_modules_expected):
                    ...
                    if not n_mod_dwords_equal:
                        module_len_fails[imod] = {
                            "expected": n_mod_dwords_expected,
                            "observed": n_mod_dwords_observed,
                        }
            l0id_module_len_fails[hex(l0id)] = module_len_fails

        n_h, n_f, n_m = (
            len(l0id_header_fails),
            len(l0id_footer_fails),
            len(l0id_n_module_fails),
        )
