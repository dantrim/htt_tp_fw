...
class BasedIntParamType(click.ParamType) :
    ...
    def convert(self, value, param, ctx) :
        try :
            ...
            self.fail(f"{value!r} is not a valid integer, expecting "
...
@click.option("-b", "--boundary", is_flag = True
        ,help = "Indicate event and module boundaries."
)
@click.option("-p", "--parse", is_flag = True
        ,help = "Print out (any) decoded information."
)
@click.option("-t", "--timestamp", is_flag = True
        ,help = "Indicate timestamps of each word in the file (if available)."
)
@click.option("-n", "--n-events", default = 0, type = int
        ,help = "Dump a specific number of events."
)
@click.option("-l", "--l0id", default = "", type = str
        ,help = "Print out a specific L0ID (can be comma-separated list for multiple, or a range indicated via <min>-<max>, e.g. 0x1-0x20)."
)
@click.option("-c", "--word-count", is_flag = True
        ,help = "Indicate the overall word count."
)
...
@click.option("--stop-l0id", type = BASED_INT
        ,help = "Do not dump events with L0ID > \"stop-l0id\"."
)
@click.option("--list-l0id", is_flag = True
        ,help = "Print L0IDs for the events in the input file."
)
def dump(
    input_file
    ,boundary
    ...
    l0id_to_load = []
    if "-" in l0id :
        l0id_to_load = list(range(int_base_convert(l0id.split("-")[0]), int_base_convert(l0id.split("-")[1])+1, 1))
    if "," in l0id :
        l0id_to_load = [int_base_convert(x.strip()) for x in l0id.split(",") if x]
    if start_l0id or stop_l0id :
        if start_l0id and stop_l0id :
            l0id_to_load = list(range(int_base_convert(start_l0id), int_base_convert(stop_l0id)+1, 1))
...
def test_summary(input
    ...
    for ifile, filename in enumerate(input_files) :
        ...
        test_passed = test_passed and str(data["test_results"]["test_success"]).lower() == "true"
    ...
    result_handler.dump_test_results( data_results, event_detail = event_detail, full_detail = detail)
    sys.exit( {True:0,False:1}[test_passed] )

@cli.command()
@click.argument("inputs", nargs = 2
)
@click.option("-n", "--n-events", default = -1, type = int
        ,help = "The number of events to load and be diffed."
)
@click.option("-e", "--endian", default = "little"
        ,type = click.Choice(["little", "big"], case_sensitive = True)
        ,help = "Endian-ness of data within the files to load."
)
@click.option("-l", "--l0id", default = -1, type = BASED_INT
...
@click.option("--event-detail", is_flag = True
...
def diff(
    ...
    diff, _ = tb_diff.compare_files(inputs[0], inputs[1], l0id
                    ,endian = endian
                    ,n_events = n_events
                    ,verbose = verbose
