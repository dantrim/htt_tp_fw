...
def main() :
    ...
    if not found :
        print("Could not find tp-fw/tb directory in current working dir (={})".format(str(pwd)))
        sys.exit(1)
    ...
    res = subprocess.run([cocotb_cfg, "--makefiles"], stdout=subprocess.PIPE, encoding="utf-8")
    if res.returncode :
        print("Unable to obtain makefile directory from cocotb-config: {}".format(res.stdout))
        sys.exit(1)
    ...
    if default_makefile_dir.is_dir() :
        ...
        if default_questa_makefile.is_file() :
            print("Using default QuestaSim makefile found in {}".format(default_makefile_dir))
    else :
        # copy cocotb one
        mkfile_name = str(questa_makefile).split("/")[-1]
        shutil.copy(questa_makefile, "default_{}".format(mkfile_name))
        print("Created default QuestaSim makefile (=\"{}\") in current directory".format(mkfile_name))

        default_questa_makefile = pwd / mkfile_name
        if default_questa_makefile.is_file() :
            print("Using default QuestaSim makefile found in {}".format(default_questa_makefile))
    ...
    print("Previous QuestaSim makefile stored at {}".format(os.path.abspath(previous_mkfile_name)))
